...
IUSE="addition debug fastcgi flv imap passenger pcre perl random-index ssl status sub webdav zlib gitorious"
...
src_install() {
	...
	if ! use gitorious ; then
		...
		cp "${FILESDIR}"/nginx.conf.gitorious conf/nginx.conf
	fi
...
pkg_postinst() {
	use ssl && {
		...
		if use gitorious ; then
			install_cert /etc/ssl/${PN}/gitorious
			chown ${PN}:${PN} "${ROOT}"/etc/ssl/${PN}/gitorious.{crt,csr,key,pem}
		fi
	}
}
